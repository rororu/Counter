...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for(;;) {
            try {
                //--- Информирую о доступных командах и предлагаю ввести команду ---
                info.commandInfo();
                info.inputValueInfo();
                ...
                if (command.equals(Command.INC.getCommandName())) {
                    ...
                    //--- Выбрасываю исключение, если команда неверная ---
                    throw new WrongCommandException();
                }
            } catch (WrongCommandException exception) {
                //--- В блоке catch ловлю исключение, вывожу сведения об ошибке и информирую пользователя ---
                System.out.println("\n" + exception.getMessage());
                info.valueInfo(counter.getValue());
            } finally {
                /* --- Блок finally сохраняет значение даже в случае
                принудительного или аварийного завершения работы приложения --- */
                counter.saveValue();
            }
        }
    }
